...
def main(msg):
    ...
    global I_dsr_vec
    # global nop 
    ###確かglobalで書いとくと関数内でその変数が変更されたとき全体に反映される
    ###この場合lambda とnumber of pixelsは定数でいいのでglobalにはかかない
    ...
    with open('./dsrth_result/desired_pose.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            desired_pose = [float(x) for x in row] ###単位mで記録されてる
    ...
    if rmse > rmseth:
        ...
        print(resvel)
        #serviceでUR手先の三次元座標の並進成分取得（tfでbase_linkからwrist_3_linkまで座標変換）
        ...
        ### desired_poseとcurrent_poseは単位メートルなので距離出すときは差の二乗の和出して1000倍して単位mmにする
    ...
    if rmse < rmseth:
        ...
        # sys.exit(0) ###ctrl c のsignal handlerと同じ終わらせ方で書いてみたけどこれだとプログラム終了しなかった
