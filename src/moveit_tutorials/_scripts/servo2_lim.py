...
def get_image(): ###tested in servo_test.py
    ...
    I_vec_arr = np.array(response.current_image) ###I_vec_arrのtypeをnumpy.ndarrayに
...
def servo():
    ...
    print('rsme = %f' % rsme)
    # print(type(pinv_int_mat))
    # # print(pinv_int_mat.shape)
    # print(type(dI))
    # print(dI.shape)
    #角速度計算
    d_vel = -lmbd*(np.dot(pinv_int_mat, dI)) 
    # print(d_vel)
    ...
    fig_rsme.savefig('./servo_data/rsme.png')
    return rsme, rsme_data, base_joint_data, shoulder_joint_data, elbow_joint_data, wrist1_joint_data, wrist2_joint_data, wrist3_joint_data
...
def main():
    ...
    fig.savefig('./servo_data/joint_values.png')
    ...
    shoulder_axis_array = np.arange(-1.3, 1.1, 0.05)
    elbow_axis_array = np.arange(1.3, 1.7, 0.05)
    wrist1_axis_array = np.arange(-2.0, -1.6, 0.05)
    wrist2_axis_array = np.arange(-1.7, -1.4, 0.05)
    ...
    #スケールを合わせたグラフを保存
    fig.savefig('./servo_data/joint_values_scaled.png')
